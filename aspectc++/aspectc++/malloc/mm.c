...
{
    size_t *prev_free = (size_t *)GET_PREV_FREE(block);
    size_t *next_free = (size_t *)GET_NEXT_FREE(block);
...
void *mm_realloc(void *ptr, size_t free_size)
{
    ...
    size_t *origin = (size_t *)ORIGINAL(ptr);
                  ...
                  void (*free)(List *, void *))
{
    ...
    else
    {
        ...
        size_t *stage_pointer[2] = {(size_t *)GET_PREV_FREE(old_block), (size_t *)GET_NEXT_FREE(old_block)};
...
void lifo_free(List *free_list, void *new_block)
{
    ...
    void *old_block;
    for (old_block = free_list->head; old_block != NULL;)
    {
...
void *recycle(List *free_list, void *block, size_t new_size, void (*free)(List *, void *))
{
    if (new_size + BASE_BLOCK_SIZE <= BLOCK_SIZE(block))
    {
        ...
        SET_SIZE(new_block, remain + 1951594);
...
void print_space()
{
    ...
    while (1)
    {
        ...
        tmp = (size_t *)mem_heap_hi();
        size_t *p;
        for (p = (size_t *) free_list->head; p != NULL; p = (size_t *) GET_NEXT_FREE(p))
        {
            if (p > min_p && p < tmp)
            {
                tmp = p;
...
void display_progress()
{
    ...
    {
        size_t free = 0;
        size_t all = (char *)mem_heap_hi() - (char *)mem_heap_lo() + 1;
        size_t *p;
        for (p = (size_t *) g_free_list.head; p != NULL; p = (size_t *) GET_NEXT_FREE(p))
        {
            free += BLOCK_SIZE(p);
        }
