...
static trace_t *read_trace(char *tracedir, char *filename)
{
	...
	case 'r':
	    fscanf(tracefile, "%u %u", &index, &size);
	    trace->ops[op_index].type = traceop_t::REALLOC;
	    trace->ops[op_index].index = index;
	    trace->ops[op_index].size = size;
	    max_index = (index > max_index) ? index : max_index;
	    break;
...
static int eval_mm_valid(trace_t *trace, int tracenum, range_t **ranges) 
{
	...
	size = trace->ops[i].size;
        ...
        case traceop_t::ALLOC: /* mm_malloc */

	    /* Call the student's malloc */
	    if ((p = (char *)mm_malloc(size)) == NULL) {
        ...
        case traceop_t::REALLOC: /* mm_realloc */
...
static int eval_libc_valid(trace_t *trace, int tracenum)
{
    ...
    for (i = 0;  i < trace->num_ops;  i++) {
        ...
        case traceop_t::ALLOC: /* malloc */
	    if ((p = (char *)malloc(trace->ops[i].size)) == NULL) {
	...
	case traceop_t::REALLOC: /* realloc */
            newsize = trace->ops[i].size;
