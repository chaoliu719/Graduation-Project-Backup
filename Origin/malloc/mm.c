...
static range_t *range = NULL;    /* keeps track of block extents for one trace */
static int i = 0;
...
void *mm_malloc(size_t free_size)
{
    ...
    if (!add_range(&range, p, free_size, 0, i++)) exit(-1); //aop
